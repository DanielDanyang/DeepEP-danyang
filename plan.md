...
| 节点 | IP | 规格 |
|------|----|------|
| `p5en_0` | `172.31.70.233` | 8× H200, 16× EFA, driver 580 |
| `p5en_1` | `172.31.71.141` | 8× H200, 16× EFA, driver 580 |

CUDA: `/usr/local/cuda-13.0`（`/usr/local/cuda` 软链可能指向旧版，构建和运行统一用 13.0）。

仓库路径：`/home/ubuntu/efs/yzhou/playground/daniel/DeepEP-danyang/`
...
export EP_JIT_CACHE_DIR=$PWD/.cache/deep_ep_jit
...
已有的构建兼容修复：
- `cccl` include 跟随 `CUDA_HOME`
- NCCL root `lib` 目录加入 `library_dirs`
- `/usr/local/cuda-13.0/bin/cuobjdump` 不存在时加了 `readelf -sW` fallback
- `csrc/jit/kernel_runtime.hpp` 补了 `<tuple>` include
...
export MASTER_ADDR=172.31.70.233
...
| MR/offset | 固定 rdma buffer | V2 buffer/workspace/scratch 注册为独立 MR，proxy `lkey_for(addr)` 查表 | 旧 command 没 region 字段 |
...
| `ProxyCtx::lkey_for(addr)` / `rkey_for(addr)` | multi-MR 地址查表，覆盖 V2 多 region |
...
| transport window | V2 buffer/workspace/scratch 注册为独立 MR，bootstrap 时 allgather rkey | 地址查 `lkey_for` / `rkey_for` |
...
| 删除原型路径 | 删除 `V2TransferCmd`、`EfaPostSink`、`V2EfaVerbsPostSink`、materialize fallback | 防止 benchmark 跑到非主路径 |
