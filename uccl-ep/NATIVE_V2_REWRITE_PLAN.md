...
| command rank/lane 语义 | V1 command decode 依赖旧 staging/QP 映射和 V1 buffer offset | V2 dispatch/combine command 都统一使用 `target_rank = scaleout rank`、`target_lane = scaleup lane` | AWS EFA proxy 面向跨节点 scaleout 传输，节点内 GPU 用 lane 区分。dispatch 和 combine 如果一个用 scaleout/lane、另一个用 global rank，会让同一 16B command 字段在两条路径上语义不同，真实 proxy 无法用同一套 endpoint table drain。 |
