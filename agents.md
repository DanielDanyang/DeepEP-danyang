...
在 AWS `p5en_0` 和 `p5en_1` 两台机器上，用隔离的 Python 虚拟环境跑通 `/home/ubuntu/efs/yzhou/playground/daniel/DeepEP-danyang/` 里的 DeepEP。单机先用 `EP_DISABLE_GIN=1` 禁用 NCCL Gin 验证；多机最终需要使用 aws-ofi-nccl master 的 GIN proxy 路径。
...
- 远端路径: `/home/ubuntu/efs/yzhou/playground/daniel/DeepEP-danyang/`
- 当前 commit: `d4f41e4`
- 检查时远端 working tree 干净
- README 中旧说明提到 `tests/utils/envs.py`，实际分布式初始化代码在 `deep_ep/utils/envs.py`
- `deep_ep/utils/envs.py` 使用环境变量：
  - `MASTER_ADDR`
  - `MASTER_PORT`
  - `WORLD_SIZE`，这里表示节点数
  - `RANK`，这里表示节点 rank
- DeepEP V2 默认使用 NCCL Gin。AWS EFA 上单机可用 `EP_DISABLE_GIN=1`，但多机必须走新的 GIN proxy 路径；禁用 Gin 的多机路径会在 symmetric/window 相关路径失败。
